...
-- Переменные для кэширования ближайшего врага
local cachedObject = nil
local shouldUpdateCache = false
...
-- Обновляем круг каждый кадр (это безопасно, так как операция дешевая)
RunService.RenderStepped:Connect(function()
	local radius = getgenv().SilentConfig.FOVRadius
...
local function getClosestObjectFromMouse(character : Model)
	local joints = {
		'Head', 'UpperTorso', "RightUpperLeg", 'RightLowerLeg',
		"LeftUpperLeg", 'LeftLowerLeg', "RightUpperArm", 'RightLowerArm',
	...
	for _, name in joints do
		...
		-- ИСПРАВЛЕНО: было dis = dis, стало dis = distance
...
-- Оптимизированная функция: вызывается только через Heartbeat, если цель еще не найдена
local function updateClosestObject()
	if not getgenv().SilentConfig.State then
		cachedObject = nil
		return
	end
	...
	cachedObject = closestObject
end

-- Обновляем кэш только один раз за кадр
RunService.Heartbeat:Connect(function()
	updateClosestObject()
end)

-- Исправленный хук: берет готовый кэш, не вычисляя каждый раз
Utility.Raycast = function(self, phem19, phem20, phem21, phem22, phem23, phem24)
	...
	local object = cachedObject -- Берем из кэша (обновляется каждый кадр)
